...
- 推断降级（实施如实化 2026-09-25）：事件驱动，无延时窗口——没有「下发后 N 秒无回执才写」的定时机制。服务端各推断点即时查询本连接的 clientPlaybackReportSet 实报集合：实报在先则 observeInferredOutcome 整体跳过推断写（账本以实报为准），无实报才按现状写推断（Source=server_inferred）。推断点本就滞后于下发，追加延时窗口只增账本延迟、不减误写，故不设。
