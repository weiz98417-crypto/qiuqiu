...
- [x] 2.2 服务端 WS 上行分发新增 playback_result 处理：FindByDeliveryKey 推进七态（Source=client）；推断路径降级为兜底（连接级 clientPlaybackReportSet 登记，各推断点事件驱动即时查询，实报在先则 observeInferredOutcome 跳过——无延时窗口机制，账本事件标 Source=server_inferred）；迟到/未知记录记 client_late 不覆盖；owner 不符拒写。
