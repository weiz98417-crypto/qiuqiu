...
- 展开器 `ExpandSubscriptions`（SweepLoop 同节拍外新增每日一次）：读活跃订阅 → 每订阅一次 14 天窗口扫描（GetFixturesContext 逐日，合并进每日 ≤2 次全窗预算）→ 队名双向 contains 命中且 kickoff 在未来 → 落 Reminder（`Kind: subscription`，去重键 = subscription_id+fixture_id，表内唯一约束语义在 Store 层查重）。
...
同名/同城前缀风险：对齐要求**双方全名相等或互为包含且长度差 ≤2**，并要求 kickoff 落在 [now-3h, now+14d]（排除历史/远期误配）；单测覆盖"曼联 U21 vs 曼联"不误配。
