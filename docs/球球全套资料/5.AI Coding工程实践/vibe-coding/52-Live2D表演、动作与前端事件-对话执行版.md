# Live2D表演、动作与前端事件：对话施工版

> 文档性质：可选舞台与多模态一致性的对话施工卡  
...
让可选角色表演只服务已获资格的短表达，并可立即降级为文字与控制，而不构成事实或关系旁路。

### 已具备的前提

事实资格、严格保护、静音、低动态和结束状态已先于任何表演资产确定。

### 本轮只能继承、不能改写的输入

舞台状态、动作计划、口型、字幕、音频、事实资格、用户控制、性能预算、低动态、不可用和结束必须共享同一决策版本。

### 可授权范围

舞台状态机、动作白名单、事件合同、性能预算、可访问替代、低动态与故障降级情境。

### 明确禁止项

强闪烁、自动高强度表演、连续注视或失落挽留、未确认事件庆祝、以动作隐藏等待、舞台独立读取或写入事实。

### 必须写入合同的结果

动作不能先于已确认且允许表达的事实；低动态、静音、严格保护和结束会令动作、口型和声音停止或退回基线；舞台失败时事实和控制仍完整可用。

### 必须 STOP 的情况

动作无法关联到资格原因；字幕与声音不同步且无法说明；低动态只是视觉开关而音频仍自动播放；性能降级会隐藏结束入口。
...
```text
主题：Live2D表演、动作与前端事件
本轮用户结果：让可选角色表演只服务已获资格的短表达，并可立即降级为文字与控制，而不构成事实或关系旁路。
已确认输入：舞台状态、动作计划、口型、字幕、音频、事实资格、用户控制、性能预算、低动态、不可用和结束必须共享同一决策版本。
仅可处理：舞台状态机、动作白名单、事件合同、性能预算、可访问替代、低动态与故障降级情境。
绝不处理：强闪烁、自动高强度表演、连续注视或失落挽留、未确认事件庆祝、以动作隐藏等待、舞台独立读取或写入事实。
请不要实现。请用普通语言复述：
...
对话施工最常见的失败是要求助手“把 Live2D表演、动作与前端事件 做完”。正确的提问是：本轮只让哪个对象从什么状态转到什么状态，谁触发，用户何处看到，谁能停止，晚到动作如何失效。使用以下表格要求助手填空，不接受“视情况而定”。
...
| 反例 | 待确认进球；严格保护；低动态；素材加载失败；音频失败；用户结束；动作重复抵达。 |
...
```text
基于已确认合同，为“Live2D表演、动作与前端事件”给出不超过五步的计划。
...
```text
只执行“Live2D表演、动作与前端事件”计划的第 1 步。
目标：<从已批准计划复制>
允许范围：<从已批准计划复制>
禁止范围：强闪烁、自动高强度表演、连续注视或失落挽留、未确认事件庆祝、以动作隐藏等待、舞台独立读取或写入事实。
硬约束：动作不能先于已确认且允许表达的事实；低动态、静音、严格保护和结束会令动作、口型和声音停止或退回基线；舞台失败时事实和控制仍完整可用。
...
待确认进球；严格保护；低动态；素材加载失败；音频失败；用户结束；动作重复抵达。
...
```mermaid
flowchart LR
    A[事实和用户控制] --> B[动态偏好与可访问性]
    B --> C[动作资格裁决]
    C --> D[前端低风险呈现]
    D --> E[交付回执与下一轮]
    A -.反例：动作泄露、无法隐藏或取消后继续.-> F[STOP、降级或回退]
    B -.-> F
    C -.-> F
    D -.-> F
```
...
若发现 动作泄露、无法隐藏或取消后继续，或任何边界不清，停止并输出 STOP 回执，不编造前提。
```
...
- 至少检查一个与“动作泄露、无法隐藏或取消后继续”相关的反例；
- 回执明确区分已运行、未运行、待验证与不能外推的结论；
- 任何新资料类别、外部写入、默认主动行为、真实用户材料或发布扩大，均返回 `00-对话式AI Coding工作流.md` 重新建立三张卡和任务契约。

## 使用边界

本篇只处理“可选舞台与动作资格”的主题任务卡。通用的对话角色、七轮施工协议、审阅纪律、交接格式和风险等级，统一使用 `00-对话式AI Coding工作流.md`；权威工程选择、正式门禁与相邻能力边界，以父目录同名工程执行篇为准。本篇的完成不证明用户价值、生产可靠性、合规或开放资格。
