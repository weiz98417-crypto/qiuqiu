# 陪看运行时、对话生成与TTS：对话施工版

> 文档性质：受控回合、生成与播放的对话施工卡  
...
在用户主动提问时交付一条可打断、不过度拟人、不越过事实和保护状态的短回合。

### 已具备的前提

事实、保护、会话和语音取消合同已定义；模型与合成服务只是可替换、可失败的外部能力。

### 本轮只能继承、不能改写的输入

用户输入、控制命令、事实资格、角色表达政策、生成候选、语音合成、播放、字幕、动作计划、取消、过期与降级必须按顺序裁决。

### 可授权范围

回合状态机、输入分类、工具调用权限、表达长度、TTS 资格、播放取消、文字降级、输出评测样本。

### 明确禁止项

模型直接写事实、自动长聊、以沉默提高主动性、角色索取照顾或排他关系、未确认事件庆祝、把完整对话作为默认长期记忆。

### 必须写入合同的结果

先识别控制与风险，再决定能否访问已确认事实；生成只是候选，输出须过边界和媒介资格；用户取消、结束、静音、保护或版本失效会使后续生成与播放无效。

### 必须 STOP 的情况

模型需要未授权资料才能作答；无法区分指令与聊天；TTS 无法取消；输出策略不能解释为什么此刻说或不说；发生高风险表达却没有收缩路径。
...
```text
主题：陪看运行时、对话生成与TTS
本轮用户结果：在用户主动提问时交付一条可打断、不过度拟人、不越过事实和保护状态的短回合。
已确认输入：用户输入、控制命令、事实资格、角色表达政策、生成候选、语音合成、播放、字幕、动作计划、取消、过期与降级必须按顺序裁决。
仅可处理：回合状态机、输入分类、工具调用权限、表达长度、TTS 资格、播放取消、文字降级、输出评测样本。
绝不处理：模型直接写事实、自动长聊、以沉默提高主动性、角色索取照顾或排他关系、未确认事件庆祝、把完整对话作为默认长期记忆。
请不要实现。请用普通语言复述：
...
对话施工最常见的失败是要求助手“把 陪看运行时、对话生成与TTS 做完”。正确的提问是：本轮只让哪个对象从什么状态转到什么状态，谁触发，用户何处看到，谁能停止，晚到动作如何失效。使用以下表格要求助手填空，不接受“视情况而定”。
...
| 反例 | 用户问待确认进球；用户说停；TTS 晚到；事实更正；静音；严格保护；角色诱导依赖的对抗提示。 |
...
```text
基于已确认合同，为“陪看运行时、对话生成与TTS”给出不超过五步的计划。
...
```text
只执行“陪看运行时、对话生成与TTS”计划的第 1 步。
目标：<从已批准计划复制>
允许范围：<从已批准计划复制>
禁止范围：模型直接写事实、自动长聊、以沉默提高主动性、角色索取照顾或排他关系、未确认事件庆祝、把完整对话作为默认长期记忆。
硬约束：先识别控制与风险，再决定能否访问已确认事实；生成只是候选，输出须过边界和媒介资格；用户取消、结束、静音、保护或版本失效会使后续生成与播放无效。
...
用户问待确认进球；用户说停；TTS 晚到；事实更正；静音；严格保护；角色诱导依赖的对抗提示。
...
```mermaid
flowchart LR
    A[用户输入和会话快照] --> B[事实与边界裁决]
    B --> C[文字回应计划]
    C --> D[声音二次资格检查]
    D --> E[交付回执与下一轮]
    A -.反例：角色越界、取消后播放或事实误说.-> F[STOP、降级或回退]
    B -.-> F
    C -.-> F
    D -.-> F
```
...
若发现 角色越界、取消后播放或事实误说，或任何边界不清，停止并输出 STOP 回执，不编造前提。
```
...
- 至少检查一个与“角色越界、取消后播放或事实误说”相关的反例；
- 回执明确区分已运行、未运行、待验证与不能外推的结论；
- 任何新资料类别、外部写入、默认主动行为、真实用户材料或发布扩大，均返回 `00-对话式AI Coding工作流.md` 重新建立三张卡和任务契约。

## 使用边界

本篇只处理“受控回合与 TTS”的主题任务卡。通用的对话角色、七轮施工协议、审阅纪律、交接格式和风险等级，统一使用 `00-对话式AI Coding工作流.md`；权威工程选择、正式门禁与相邻能力边界，以父目录同名工程执行篇为准。本篇的完成不证明用户价值、生产可靠性、合规或开放资格。
