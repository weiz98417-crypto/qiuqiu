# Evals评测、黄金集与自动回归：对话施工版

> 文档性质：可反驳质量门与黄金情境的对话施工卡  
...
建立可版本化的黄金集、反例、评分量表与阻断门，验证事实、控制、关系、资料和降级在异常条件下仍保持一致。

### 已具备的前提

产品与工程合同已经明确；评测要检验承诺，不把好听的单轮回复当作质量证据。

### 本轮只能继承、不能改写的输入

事实可信、用户主权、关系边界、语音/多端可用、资料最小化和韧性是六个独立维度；每个样本有输入条件、预期状态、禁止输出、判定理由与风险级别。

### 可授权范围

黄金场景、标签规则、评分表、离线评测、集成情境、端到端演练、回归门与报告模板。

### 明确禁止项

只测顺畅回答、用总体分掩盖高风险失败、用真实敏感用户内容造集、让模型自己生成并判定全部样本、以使用量替代安全门。

### 必须写入合同的结果

每次合同变化需增加正例与反例；高风险 P0/P1 任一失败阻断相关开放；样本与预期版本化，未知项明确标注而非静默放宽。

### 必须 STOP 的情况

无法说清某个分数对应用户后果；样本没有来源/目的；严格保护、结束、删除或依赖风险未覆盖；评分者与施工者没有独立视角。
...
```text
主题：Evals评测、黄金集与自动回归
本轮用户结果：建立可版本化的黄金集、反例、评分量表与阻断门，验证事实、控制、关系、资料和降级在异常条件下仍保持一致。
已确认输入：事实可信、用户主权、关系边界、语音/多端可用、资料最小化和韧性是六个独立维度；每个样本有输入条件、预期状态、禁止输出、判定理由与风险级别。
仅可处理：黄金场景、标签规则、评分表、离线评测、集成情境、端到端演练、回归门与报告模板。
绝不处理：只测顺畅回答、用总体分掩盖高风险失败、用真实敏感用户内容造集、让模型自己生成并判定全部样本、以使用量替代安全门。
请不要实现。请用普通语言复述：
...
对话施工最常见的失败是要求助手“把 Evals评测、黄金集与自动回归 做完”。正确的提问是：本轮只让哪个对象从什么状态转到什么状态，谁触发，用户何处看到，谁能停止，晚到动作如何失效。使用以下表格要求助手填空，不接受“视情况而定”。
...
| 反例 | 待确认事实；严格保护旁路；语音取消；删除晚到写入；关系诱导；断线重连；无障碍与低动态。 |
...
```text
基于已确认合同，为“Evals评测、黄金集与自动回归”给出不超过五步的计划。
...
```text
只执行“Evals评测、黄金集与自动回归”计划的第 1 步。
目标：<从已批准计划复制>
允许范围：<从已批准计划复制>
禁止范围：只测顺畅回答、用总体分掩盖高风险失败、用真实敏感用户内容造集、让模型自己生成并判定全部样本、以使用量替代安全门。
硬约束：每次合同变化需增加正例与反例；高风险 P0/P1 任一失败阻断相关开放；样本与预期版本化，未知项明确标注而非静默放宽。
...
待确认事实；严格保护旁路；语音取消；删除晚到写入；关系诱导；断线重连；无障碍与低动态。
...
flowchart LR
    ...
    C --> D[CI 门禁与报告]
    D --> E[交付回执与下一轮]
    A -.反例：删场景保分、硬门被软评分覆盖或无法复现.-> F[STOP、降级或回退]
    B -.-> F
    C -.-> F
    D -.-> F
```
...
若发现 删场景保分、硬门被软评分覆盖或无法复现，或任何边界不清，停止并输出 STOP 回执，不编造前提。
```
...
- 至少检查一个与“删场景保分、硬门被软评分覆盖或无法复现”相关的反例；
- 回执明确区分已运行、未运行、待验证与不能外推的结论；
- 任何新资料类别、外部写入、默认主动行为、真实用户材料或发布扩大，均返回 `00-对话式AI Coding工作流.md` 重新建立三张卡和任务契约。

## 使用边界

本篇只处理“黄金集与工程回归”的主题任务卡。通用的对话角色、七轮施工协议、审阅纪律、交接格式和风险等级，统一使用 `00-对话式AI Coding工作流.md`；权威工程选择、正式门禁与相邻能力边界，以父目录同名工程执行篇为准。本篇的完成不证明用户价值、生产可靠性、合规或开放资格。
