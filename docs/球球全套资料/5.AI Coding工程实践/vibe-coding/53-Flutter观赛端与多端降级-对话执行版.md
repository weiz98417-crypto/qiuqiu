# Flutter观赛端与多端降级：对话施工版

> 文档性质：跨端用户控制与无障碍的对话施工卡  
...
让 Web 和移动端用户都能在三秒内看见当前事实状态与停止入口，并在声音、舞台、网络或性能失败时保留完整文字路径。

### 已具备的前提

用户任务、事实状态、保护与结束合同已确定；Flutter 是首轮跨端选择而非自动保证所有平台一致。

### 本轮只能继承、不能改写的输入

进入场次、事实快照、等待/冲突、更正、严格保护、静音、文字优先、低动态、对话、语音、舞台、重连和结束是可组合但有优先级的界面状态。

### 可授权范围

Flutter 组件层级、语义标签、键盘/触控路径、平台能力探测、响应式断点、降级文案、可用性场景。

### 明确禁止项

要求麦克风或角色加载后才能看事实、把状态只用颜色表达、在小屏隐藏结束、以动态效果代替文字解释、通过端能力猜测用户意图。

### 必须写入合同的结果

事实与控制先渲染；所有关键操作有可访问名称与非手势替代；平台不支持的媒介显示中性不可用说明并保持文字任务；严格保护跨组件一致。

### 必须 STOP 的情况

一项核心任务只有一个输入方式；状态无文本含义；Web/移动端在结束和保护上语义不同；性能问题迫使用户无法静音或结束。
...
```text
主题：Flutter观赛端与多端降级
本轮用户结果：让 Web 和移动端用户都能在三秒内看见当前事实状态与停止入口，并在声音、舞台、网络或性能失败时保留完整文字路径。
已确认输入：进入场次、事实快照、等待/冲突、更正、严格保护、静音、文字优先、低动态、对话、语音、舞台、重连和结束是可组合但有优先级的界面状态。
仅可处理：Flutter 组件层级、语义标签、键盘/触控路径、平台能力探测、响应式断点、降级文案、可用性场景。
绝不处理：要求麦克风或角色加载后才能看事实、把状态只用颜色表达、在小屏隐藏结束、以动态效果代替文字解释、通过端能力猜测用户意图。
请不要实现。请用普通语言复述：
...
对话施工最常见的失败是要求助手“把 Flutter观赛端与多端降级 做完”。正确的提问是：本轮只让哪个对象从什么状态转到什么状态，谁触发，用户何处看到，谁能停止，晚到动作如何失效。使用以下表格要求助手填空，不接受“视情况而定”。
...
| 反例 | 小屏横屏；屏幕阅读器；键盘导航；舞台加载失败；拒绝麦克风；弱网；严格保护；共享设备。 |
...
```text
基于已确认合同，为“Flutter观赛端与多端降级”给出不超过五步的计划。
...
```text
只执行“Flutter观赛端与多端降级”计划的第 1 步。
目标：<从已批准计划复制>
允许范围：<从已批准计划复制>
禁止范围：要求麦克风或角色加载后才能看事实、把状态只用颜色表达、在小屏隐藏结束、以动态效果代替文字解释、通过端能力猜测用户意图。
硬约束：事实与控制先渲染；所有关键操作有可访问名称与非手势替代；平台不支持的媒介显示中性不可用说明并保持文字任务；严格保护跨组件一致。
...
小屏横屏；屏幕阅读器；键盘导航；舞台加载失败；拒绝麦克风；弱网；严格保护；共享设备。
...
```mermaid
flowchart LR
    A[比赛状态和控制] --> B[Flutter 展示状态]
    B --> C[文字与无障碍路径]
    C --> D[可选声音和舞台]
    D --> E[交付回执与下一轮]
    A -.反例：平台差异、弱网或媒介失效.-> F[STOP、降级或回退]
    B -.-> F
    C -.-> F
    D -.-> F
```
...
若发现 平台差异、弱网或媒介失效，或任何边界不清，停止并输出 STOP 回执，不编造前提。
```
...
- 至少检查一个与“平台差异、弱网或媒介失效”相关的反例；
- 回执明确区分已运行、未运行、待验证与不能外推的结论；
- 任何新资料类别、外部写入、默认主动行为、真实用户材料或发布扩大，均返回 `00-对话式AI Coding工作流.md` 重新建立三张卡和任务契约。

## 使用边界

本篇只处理“跨端文字优先与降级”的主题任务卡。通用的对话角色、七轮施工协议、审阅纪律、交接格式和风险等级，统一使用 `00-对话式AI Coding工作流.md`；权威工程选择、正式门禁与相邻能力边界，以父目录同名工程执行篇为准。本篇的完成不证明用户价值、生产可靠性、合规或开放资格。
