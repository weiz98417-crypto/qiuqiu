# 语音输入、VAD与ASR链路：对话施工版

> 文档性质：主动语音、取消与文字降级的对话施工卡  
...
让用户主动开启一次可随时取消的语音回合，并在许可、识别、网络或性能失败时完整退回文字路径。

### 已具备的前提

用户端文字路径、结束控制和资料最小化原则已成立；音频供应与设备能力仍需逐平台验证。

### 本轮只能继承、不能改写的输入

用户动作、平台许可、本地采集、语音活动判定、临时分段、转写候选、确认、取消、结束、严格保护和删除是不同阶段；声音不是持续开放入口。

### 可授权范围

语音状态机、许可文案、VAD 预算、分段合同、转写采用条件、取消传播、字幕/文字替代和测试音频。

### 明确禁止项

后台监听、默认录音、保存原始声音、用环境声或声纹推断用户、把未确认转写写成长期资料、取消后补播或补写。

### 必须写入合同的结果

录音只在显式用户动作后进行；取消必须使采集、上传、识别、生成、合成和播放失效；原始声音默认不保存；严格保护对语音事实表达同样有效。

### 必须 STOP 的情况

无法说明录音指示或停止入口；平台许可拒绝后没有文字路径；转写来源与保留范围不清；在途音频无法取消；背景声可被当作指令。
...
```text
主题：语音输入、VAD与ASR链路
本轮用户结果：让用户主动开启一次可随时取消的语音回合，并在许可、识别、网络或性能失败时完整退回文字路径。
已确认输入：用户动作、平台许可、本地采集、语音活动判定、临时分段、转写候选、确认、取消、结束、严格保护和删除是不同阶段；声音不是持续开放入口。
仅可处理：语音状态机、许可文案、VAD 预算、分段合同、转写采用条件、取消传播、字幕/文字替代和测试音频。
绝不处理：后台监听、默认录音、保存原始声音、用环境声或声纹推断用户、把未确认转写写成长期资料、取消后补播或补写。
请不要实现。请用普通语言复述：
...
对话施工最常见的失败是要求助手“把 语音输入、VAD与ASR链路 做完”。正确的提问是：本轮只让哪个对象从什么状态转到什么状态，谁触发，用户何处看到，谁能停止，晚到动作如何失效。使用以下表格要求助手填空，不接受“视情况而定”。
...
| 反例 | 许可拒绝；嘈杂背景；用户打断；网络中断；转写晚到；严格保护中询问比分；结束后合成返回。 |
...
```text
基于已确认合同，为“语音输入、VAD与ASR链路”给出不超过五步的计划。
...
```text
只执行“语音输入、VAD与ASR链路”计划的第 1 步。
目标：<从已批准计划复制>
允许范围：<从已批准计划复制>
禁止范围：后台监听、默认录音、保存原始声音、用环境声或声纹推断用户、把未确认转写写成长期资料、取消后补播或补写。
硬约束：录音只在显式用户动作后进行；取消必须使采集、上传、识别、生成、合成和播放失效；原始声音默认不保存；严格保护对语音事实表达同样有效。
...
许可拒绝；嘈杂背景；用户打断；网络中断；转写晚到；严格保护中询问比分；结束后合成返回。
...
任务卡 A：只建立语音状态与用户可见提示。

任务卡 B：只验证取消传播，不接入长期保存。
...
flowchart LR
    ...
    C --> D[可选回应]
    D --> E[交付回执与下一轮]
    A -.反例：取消失效、权限拒绝或网络故障.-> F[STOP、降级或回退]
    B -.-> F
    C -.-> F
    D -.-> F
```
...
若发现 取消失效、权限拒绝或网络故障，或任何边界不清，停止并输出 STOP 回执，不编造前提。
```
...
- 至少检查一个与“取消失效、权限拒绝或网络故障”相关的反例；
- 回执明确区分已运行、未运行、待验证与不能外推的结论；
- 任何新资料类别、外部写入、默认主动行为、真实用户材料或发布扩大，均返回 `00-对话式AI Coding工作流.md` 重新建立三张卡和任务契约。

## 使用边界

本篇只处理“主动语音与文字降级”的主题任务卡。通用的对话角色、七轮施工协议、审阅纪律、交接格式和风险等级，统一使用 `00-对话式AI Coding工作流.md`；权威工程选择、正式门禁与相邻能力边界，以父目录同名工程执行篇为准。本篇的完成不证明用户价值、生产可靠性、合规或开放资格。
