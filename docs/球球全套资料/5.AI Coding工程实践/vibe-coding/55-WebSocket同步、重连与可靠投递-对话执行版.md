# WebSocket同步、重连与可靠投递：对话施工版

> 文档性质：实时快照、乱序与取消优先的对话施工卡  
...
让每个有效会话先获得可解释快照，再接收最小增量；重复、乱序、断线和迟到动作不产生额外用户副作用。

### 已具备的前提

会话范围、事实版本、用户控制和结束语义已明确；实时连接只是传输，不拥有业务判断。

### 本轮只能继承、不能改写的输入

连接票据、认证、会话范围、快照版本、增量序列、确认、重连游标、取消、结束、删除、更正和降级分开。

### 可授权范围

消息信封、订阅范围、顺序/去重规则、重连策略、快照回补、心跳、错误文案和网络反例。

### 明确禁止项

按场次编号订阅他人资料、把连接成功当作状态可信、重放旧声音或动作、结束后继续投递、以重试绕过删除或保护状态。

### 必须写入合同的结果

服务先验证范围再发送快照；客户端以版本与有效控制过滤增量；无法安全续接时丢弃增量并重取快照；取消、结束、删除和权限失效高于重试。

### 必须 STOP 的情况

消息没有会话/版本语义；重连可见旧状态倒退；无法解释重复副作用如何避免；订阅范围来自可猜测标识；断线时没有保守降级。
...
```text
主题：WebSocket同步、重连与可靠投递
本轮用户结果：让每个有效会话先获得可解释快照，再接收最小增量；重复、乱序、断线和迟到动作不产生额外用户副作用。
已确认输入：连接票据、认证、会话范围、快照版本、增量序列、确认、重连游标、取消、结束、删除、更正和降级分开。
仅可处理：消息信封、订阅范围、顺序/去重规则、重连策略、快照回补、心跳、错误文案和网络反例。
绝不处理：按场次编号订阅他人资料、把连接成功当作状态可信、重放旧声音或动作、结束后继续投递、以重试绕过删除或保护状态。
请不要实现。请用普通语言复述：
...
对话施工最常见的失败是要求助手“把 WebSocket同步、重连与可靠投递 做完”。正确的提问是：本轮只让哪个对象从什么状态转到什么状态，谁触发，用户何处看到，谁能停止，晚到动作如何失效。使用以下表格要求助手填空，不接受“视情况而定”。
...
| 反例 | 重复消息；乱序更正；后台恢复；网络中断；结束后晚到；删除竞争；严格保护切换。 |
...
```text
基于已确认合同，为“WebSocket同步、重连与可靠投递”给出不超过五步的计划。
...
```text
只执行“WebSocket同步、重连与可靠投递”计划的第 1 步。
目标：<从已批准计划复制>
允许范围：<从已批准计划复制>
禁止范围：按场次编号订阅他人资料、把连接成功当作状态可信、重放旧声音或动作、结束后继续投递、以重试绕过删除或保护状态。
硬约束：服务先验证范围再发送快照；客户端以版本与有效控制过滤增量；无法安全续接时丢弃增量并重取快照；取消、结束、删除和权限失效高于重试。
...
重复消息；乱序更正；后台恢复；网络中断；结束后晚到；删除竞争；严格保护切换。
...
flowchart LR
    ...
    C --> D[重连后的最小恢复]
    D --> E[交付回执与下一轮]
    A -.反例：旧消息复活、重复副作用或结束后重连.-> F[STOP、降级或回退]
    B -.-> F
    C -.-> F
    D -.-> F
```
...
若发现 旧消息复活、重复副作用或结束后重连，或任何边界不清，停止并输出 STOP 回执，不编造前提。
```
...
- 至少检查一个与“旧消息复活、重复副作用或结束后重连”相关的反例；
- 回执明确区分已运行、未运行、待验证与不能外推的结论；
- 任何新资料类别、外部写入、默认主动行为、真实用户材料或发布扩大，均返回 `00-对话式AI Coding工作流.md` 重新建立三张卡和任务契约。

## 使用边界

本篇只处理“快照、增量与取消优先”的主题任务卡。通用的对话角色、七轮施工协议、审阅纪律、交接格式和风险等级，统一使用 `00-对话式AI Coding工作流.md`；权威工程选择、正式门禁与相邻能力边界，以父目录同名工程执行篇为准。本篇的完成不证明用户价值、生产可靠性、合规或开放资格。
