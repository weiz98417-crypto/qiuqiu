...
承载同一轮调和相关的事实成员与冲突边。集合本身是展示和审计容器，不代表整组事实只能选择一条；只有存在直接冲突边的事实互斥，没有冲突边的事实可以同时保留。冲突集合有独立状态，不再借用某条旧事实的公开状态表达。
...
- 新增冲突表和写入流程。
- 新冲突不再修改原已接受事实。
- 导演台接入兼容事实多选，默认保留当前已接受事实。
- 调和通过单事务切换公开权并发布一次投影差异。

退出条件：开放冲突期间用户端状态稳定；调和后保留全部兼容事实，只撤销与所选事实直接互斥的事实。

实施状态（2026-07-21）：**已完成并修正为冲突图语义**。`024_fact_conflict_graph.sql` 新增 `fact_conflict_edges` 和 `fact_conflict_resolution_facts`，旧冲突按 accepted × candidate 保守补边；`FactConflict` 对外返回 `edges` 与 `selectedFactIds`。导演台通过 `POST /api/matches/{matchId}/conflicts/{conflictId}/resolve` 提交兼容事实集合，旧 `chosenFactId` 请求会自动扩展为与所选事实兼容的集合。内存与 PostgreSQL 共用纯领域解析器：完整解析时关闭冲突，部分解析时只删除已解决边并保留剩余开放冲突；只撤销与所选事实直接互斥的事实。冲突创建、成员/边写入、事实切换、完整性恢复和 outbox 信号仍位于同一事务。导演台默认勾选全部 accepted 事实，选择候选时只取消与其直接互斥的选项；浏览器回归覆盖多 accepted 默认保留和兼容事实不被桥接候选误撤销。
...
7. 同一冲突集合中的两个 accepted 事实默认同时保留。
8. 选择桥接候选时只撤销与其存在直接冲突边的事实，兼容事实继续公开。
9. 部分调和后剩余冲突边继续开放，比赛完整性保持 `conflict`。
