...
事件写入、幂等记录和 Outbox 消息在一个数据库事务中完成。后台发布器读取 Outbox，成功发送后标记完成，失败则按退避策略重试。客户端收到重复消息时使用 `deliveryKey = eventId:factRevision:factStatus` 去重，因此精确重复会被抑制，合法事实修订仍会继续展示和播报。
...
当前阶段满足演示和单进程服务的幂等验收，但不宣称所有外部副作用都具备跨崩溃 exactly-once：

- 比赛事件、事实修订和 Outbox 写入共享数据库事务，失败时整体回滚。
- 配置、自动化、信号源、人工接管和演示重置先持久化预占记录，再执行副作用；失败会记录 `failed`，中断的 `pending` 会在租约过期后恢复。若副作用已经成功、服务却在完成记录前崩溃，恢复后仍可能再次执行。
- Outbox 当前保证发布到进程内订阅队列，不包含 WebSocket 客户端 ACK、持久化重放游标或跨设备投递账本。

正式上线前需要明确产品承诺：若接受“至少一次 + 客户端去重”的可靠性，当前协议可继续使用；若要求强可靠主动播报和外部副作用 exactly-once，需要增加持久化命令消费者、操作级状态对账，以及客户端 ACK/重放游标。
...
- [x] 用两组真实用户会话验证 WebSocket、Trace、隐私接口和导播接口的数据隔离。
- [x] 验证候选事件、确认事件、撤销事件、修订事件和再次确认的完整事实生命周期。
- [x] 用用户端验证比分、问答、主动播报、轮播和同一事实修订的投递一致性。
- [x] 覆盖重复点击、网络重试等价重复投递、服务重启和删除期间异步任务。
- [x] 本地演示环境以 `AUTH_MODE=session` 运行并完成验收。
- [ ] 生产灰度需先补客户端 ACK/持久化重放游标，并决定非事务外部副作用的跨崩溃语义。

阶段 5 的测试矩阵、执行命令和灰度门槛见 `docs/phase5-validation-report.md`。
