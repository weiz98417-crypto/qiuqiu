...
长期记忆参考与用户画像只描述用户背景，可以在相关时自然带过，不得当作赛况事实，不得据此编造比赛细节或新增事实。
...
称呼：%s`,
				...
				portraitContextOrNone(req.PortraitContext),
				content.Goal,
				req.ReliableText,
...
// portraitContextOrNone mirrors memoryContextOrNone for the user-model block:
// an unconfigured seam, a degraded adapter, or a just-forgotten portrait all
// render as 无.
func portraitContextOrNone(portraitContext string) string {
	if strings.TrimSpace(portraitContext) == "" {
		return "无"
	}
